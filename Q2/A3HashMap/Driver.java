...
class TestProcess extends Thread {
	...
	while( !agg.isFinished() ) {
		...
		// Lookups. Do a series of gets per
		// add/remove pair to reflect typical usage.
		for( int l=0; l < num_gets && !agg.isRoundDone(); ++l, ++v ) {
		    int kg = rng.nextInt(num_elems+(num_elems>>3));
		    if( kg >= num_elems ) {
			// An unsuccessful lookup (~10% of all gets)
...
class Driver {
	...
	if( args.length < 7 ) {
	    System.err.println("Usage: Driver <num_threads> <init-size> <set-size> <gets> <delay> <rounds> <type>");
	    System.exit( 1 );
	}
	...
	int num_gets = parse_integer( args, 4 );
